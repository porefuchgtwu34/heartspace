...
function ConversationListItem({
  ...
  return (
    <li>
      ...
      >
        <UserAvatar username={other.username} avatarUrl={other.avatarUrl} size="md" online={online} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className={cn('font-medium truncate', active ? 'text-rose-700 dark:text-rose-300' : 'text-foreground')}>
              @{other.username}
            </span>
            {lastMessage && (
              <span className="text-[10px] text-muted-foreground shrink-0">{timeAgo(lastMessage.createdAt)}</span>
            )}
          </div>
          <div className="flex items-center justify-between gap-2 mt-0.5">
            <span className={cn('text-xs truncate', conversation.unread > 0 && !active ? 'text-foreground font-medium' : 'text-muted-foreground')}>
              {preview}
            </span>
...
function ChatHeader({
  ...
  return (
    <div className="flex items-center gap-3 px-3 sm:px-5 py-3 border-b border-border/60 bg-card/60 backdrop-blur-sm">
      <button onClick={onBack} className="md:hidden -ml-1 grid h-8 w-8 place-items-center rounded-full text-muted-foreground hover:bg-muted hover:text-foreground transition" aria-label="Back to conversations">
        <ArrowLeft className="h-4 w-4" />
      </button>
      <UserAvatar username={conversation.other.username} avatarUrl={conversation.other.avatarUrl} size="md" online={online} />
      <div className="min-w-0 flex-1">
        <h3 className="font-display font-semibold truncate leading-none">@{conversation.other.username}</h3>
        <div className="mt-1 h-4 flex items-center">
          <AnimatePresence mode="wait" initial={false}>
            {typing ? (
              <motion.span key="typing" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="text-xs text-rose-600 dark:text-rose-400 inline-flex items-center gap-1">
                <span className="flex gap-0.5">
                  <span className="h-1 w-1 rounded-full bg-rose-500 animate-bounce" style={{ animationDelay: '0ms' }} />
                  <span className="h-1 w-1 rounded-full bg-rose-500 animate-bounce" style={{ animationDelay: '120ms' }} />
                  <span className="h-1 w-1 rounded-full bg-rose-500 animate-bounce" style={{ animationDelay: '240ms' }} />
                </span>
                typing…
              </motion.span>
            ) : (
              <motion.span key="status" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="text-xs text-muted-foreground inline-flex items-center gap-1">
                {online ? (<><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Online</>) : (<>Offline</>)}
              </motion.span>
            )}
          </AnimatePresence>
        </div>
...
function MessageList({ messages, currentUserId }: { messages: ChatMessage[]; currentUserId: string }) {
      ...
      {groups.map((g) => (
        <div key={g.key} className="space-y-1.5">
          <div className="sticky top-0 z-10 my-2 flex justify-center">
            <span className="rounded-full bg-muted/80 px-2.5 py-0.5 text-[10px] font-medium text-muted-foreground backdrop-blur-sm shadow-sm">{g.label}</span>
          </div>
          {g.items.map((m, idx) => {
            const mine = m.senderId === currentUserId
            const prev = g.items[idx - 1]
            const showAvatar = !mine && (!prev || prev.senderId !== m.senderId)
            return <MessageBubble key={m.id} message={m} mine={mine} showAvatar={showAvatar} />
          })}
        </div>
      ))}
    </div>
...
function MessageBubble({ message, mine, showAvatar }: { message: ChatMessage; mine: boolean; showAvatar: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.18, ease: 'easeOut' }} className={cn('flex items-end gap-2', mine ? 'flex-row-reverse' : 'flex-row')}>
      <div className="w-8 shrink-0">
        {showAvatar && <UserAvatar username={message.sender.username} avatarUrl={message.sender.avatarUrl} size="xs" />}
      </div>
      <div className={cn('max-w-[78%] sm:max-w-[68%] flex flex-col', mine ? 'items-end' : 'items-start')}>
        <div className={cn('rounded-2xl px-3.5 py-2 text-sm break-words shadow-sm whitespace-pre-wrap', mine ? 'bg-gradient-to-br from-rose-500 to-fuchsia-600 text-white rounded-br-md' : 'bg-card border border-border/60 text-card-foreground rounded-bl-md')}>
          {message.content}
        </div>
        <span className="mt-1 px-1 text-[10px] text-muted-foreground/70">{formatTime(message.createdAt)}</span>
      </div>
    </motion.div>
  )
}

function ChatInput({ draft, onChange, onKeyDown, onSend, sending }: { draft: string; onChange: (v: string) => void; onKeyDown: (e: React.KeyboardEvent<HTMLTextAreaElement>) => void; onSend: () => void; sending: boolean }) {
  return (
    <div className="border-t border-border/60 bg-card/60 backdrop-blur-sm p-3">
      <div className="flex items-end gap-2 rounded-2xl border border-border/60 bg-background px-3 py-2 transition focus-within:ring-2 focus-within:ring-rose-500/30 focus-within:border-rose-500/40">
        <Textarea value={draft} onChange={(e) => onChange(e.target.value)} onKeyDown={onKeyDown} placeholder="Write something kind…" rows={1} className="min-h-9 max-h-32 resize-none border-0 bg-transparent p-0 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-transparent text-sm" />
        <Button size="icon" onClick={onSend} disabled={!draft.trim() || sending} className="h-9 w-9 shrink-0 rounded-full bg-gradient-to-r from-rose-500 to-fuchsia-600 hover:from-rose-600 hover:to-fuchsia-700 text-white shadow-md shadow-rose-500/30 disabled:opacity-40 disabled:shadow-none transition-transform hover:scale-105 active:scale-95" aria-label="Send message">
...
function EmptyChatHero() {
  return (
    <div className="flex flex-1 flex-col items-center justify-center text-center px-6 py-12">
      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="relative mb-6">
      ...
      <h3 className="font-display text-2xl font-semibold">Choose a heart to write to <span className="inline-block">💕</span></h3>
      <p className="mt-2 max-w-sm text-sm text-muted-foreground">Pick a conversation from the left, or start a new one. Every message here is private and yours.</p>
    </div>
  )
}
...
function EmptyChat({ name }: { name: string }) {
  return (
    <div className="flex h-full flex-col items-center justify-center text-center px-6">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-rose-500/5 px-6 py-5 ring-1 ring-rose-500/15">
        <Heart className="mx-auto h-7 w-7 text-rose-500 fill-rose-500/20" />
        <p className="mt-2 text-sm">This is the start of your conversation with <span className="font-semibold text-rose-600 dark:text-rose-400">@{name}</span>.</p>
...
function NewMessageDialog({ open, onOpenChange, onPick }: { open: boolean; onOpenChange: (v: boolean) => void; onPick: (userId: string) => void }) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Search for someone by username to start a private conversation.</DialogDescription>
        </DialogHeader>
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="e.g. lavender_dreams" className="pl-9" />
        </div>
        <div className="max-h-72 overflow-y-auto scroll-soft -mx-1">
          {loading ? (
            ...
            <div className="px-4 py-10 text-center">
              ...
              <p className="text-sm">{q ? `No one found for "${q}".` : 'Start typing to find someone.'}</p>
            </div>
          ) : (
            <ul className="space-y-0.5 p-1">
              {results.map((u) => (
                <li key={u.id}>
                  <button onClick={() => onPick(u.id)} className="flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left hover:bg-rose-500/10 transition-colors">
